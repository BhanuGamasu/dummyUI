    ...
    <div className="p-6">
      ...
      <div className="space-y-6">
        {orders.map((order) => (
          ...
          >
            {/* First Product Image */}
            <div className="w-28 h-28 flex-shrink-0" onClick={() => handleOrderClick(order.id)}>
              <img
              ...
              <div onClick={() => handleOrderClick(order.id)}>
                ...
                <div className="flex justify-between items-center mt-1">
                  <p className="text-sm text-gray-800 flex items-center">
                    <span className="text-lg text-gray-800 font-semibold p-1">₹{order.totalAmount}</span>
                  </p>
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <motion.button
                        ...
                        whileTap={{ scale: 1.2 }}
                        className={`text-3xl ${ratings[order.id] && ratings[order.id] >= star
                          ? "text-green-500"
                          : "text-gray-300"
                          }`}
                      >
